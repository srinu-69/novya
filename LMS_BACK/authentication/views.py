...
def register_user(request):
    ...
    if serializer.is_valid():
        ...
        # Create role-specific profile (simplified for now)
        if user.role == 'Student':
            # For now, we'll create a Student record without class_field and parent
            # These can be added later through separate endpoints
            try:
                Student.objects.create(student=user)
            except Exception as e:
                # If Student creation fails, we still have the user
                pass
        elif user.role == 'Parent':
            try:
                Parent.objects.create(parent=user)
            except Exception as e:
                # If Parent creation fails, we still have the user
                pass
...
def update_user_profile(request):
    ...
    try:
        ...
        try:
            ...
            student_registration.parent_email = request.data.get('parentEmail', student_registration.parent_email)
            ...
            if not created:
                ...
                student_profile.parent_email = request.data.get('parentEmail', student_profile.parent_email)
