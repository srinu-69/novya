...
const Test = () => {
  ...
  const subjects = [
    // Computer subjects (unchanged - use old endpoints)
    {
      ...
      apiEndpoint: 'programming_language',
      subject: 'computer',
      isRandomized: false
    },
    {
    ...
    },
    {
      ...
      apiEndpoint: 'microsoft_access',
      subject: 'computer',
      isRandomized: false
    },
    // Maths subjects (NEW - use randomized endpoints)
    {
      id: 'maths-integers',
      ...
      apiEndpoint: 'chapter1_integers',
      subject: 'maths',
      isRandomized: true
    }
  ];

  // API function to load quiz questions from backend
  const loadQuizQuestions = async (subject) => {
    setLoading(true);
    setError(null);
    ...
    try {
      ...
      if (subject.isRandomized && subject.subject === 'maths') {
        // Use randomized endpoint for Maths
        endpoint = `/api/quizzes/maths/class7/${subject.subject}/${subject.apiEndpoint}/randomized/`;
      } else {
        // Use regular endpoint for Computer and other subjects
        endpoint = `/api/quizzes/pdf/class7/${subject.subject}/${subject.apiEndpoint}/questions/`;
      }

      console.log(`Loading quiz from endpoint: ${endpoint}`);

      // Don't send auth headers for randomized Maths quizzes
      const headers = (subject.isRandomized && subject.subject === 'maths')
  ...
  const submitQuizAnswers = async (subject, answers) => {
    try {
      let endpoint;

      // Use different endpoints based on subject type
      if (subject.isRandomized && subject.subject === 'maths') {
        // Use randomized submit endpoint for Maths
        endpoint = `/api/quizzes/maths/class7/${subject.subject}/${subject.apiEndpoint}/randomized/submit/`;
      } else {
        // Use regular submit endpoint for Computer and other subjects
        endpoint = `/api/quizzes/pdf/class7/${subject.subject}/${subject.apiEndpoint}/submit/`;
      }
  ...
  const handleSubjectSelect = async (subject) => {
    ...
    await loadQuizQuestions(subject);
  };
  ...
  const handleFinishQuiz = async () => {
    ...
    if (quizData && quizData.questions) {
      try {
        ...
        const answers = Object.entries(userAnswers).map(([questionIndex, selectedOption]) => ({
          question_id: parseInt(questionIndex) + 1,
          selected_option: selectedOption
        }));

        // Submit answers to API
        const result = await submitQuizAnswers(selectedSubject, answers);

        // Update score with API response
  ...
  const currentQuestions = quizData ? quizData.questions : [];
              ...
              return (
                <div key={question.id} className={`result-item ${isCorrect ? 'correct' : 'incorrect'}`}>
                  ...
                  <div className="result-answer">
                    ...
                    {!isCorrect && <p>Correct answer: {question.correct_option}</p>}
                  </div>
      ...
      {loading ? (
        <div className="quiz-loading">
          <div className="loading-card">
            ...
            <p>Please wait while we load the questions from your PDFs.</p>
        ...
        <div className="quiz-error">
          <div className="error-card">
            ...
            <p>{error}</p>
            <button onClick={() => loadQuizQuestions(selectedSubject)} className="btn-retry">
              <RotateCcw size={18} />
              Try Again
            </button>
          </div>
        ...
        <div className="quiz-instructions">
          <div className="instructions-card">
            <h3>Instructions</h3>
            {selectedSubject.isRandomized && (
              <div className="randomized-notice" style={{
                backgroundColor: '#e3f2fd',
                ...
                <strong style={{ color: '#1976d2' }}>🎲 Randomized Quiz</strong>
            ...
            <ul>
              ...
              {selectedSubject.isRandomized && (
                <li><strong>Note:</strong> Each time you retake this quiz, you'll get different questions!</li>
              )}
            </ul>

            <div className="instructions-actions">
              <button onClick={handleStartQuiz} className="btn-start" disabled={!quizData}>
                <Play size={18} />
                Start Quiz
              </button>
            </div>
          ...
          <div className="question-card">
            ...
            <div className="options-container">
              {currentQuestionData.options.map((option, index) => (
                <div
                  key={index}
                  className={`option ${selectedAnswer === option.option_id ? 'selected' : ''}`}
                ...
                >
                  ...
                  <span>{option.option_id}. {option.option_text}</span>
                </div>
              ))}
            </div>
